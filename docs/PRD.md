...
| V2.0.2 | 2026-03-24 | HealthAI Team | 对齐 PRD 与功能结构图的优先级、状态口径与验收标准 | PM 评审修订 |

---

## 1.1 文档定位与使用边界

- 本文档是 **产品范围与验收基线**，用于定义当前版本要解决什么问题、哪些能力属于优先范围、以及什么情况下可以认为某个产品能力已达到“可验收”标准。
- [FEATURE_MAP.md](./FEATURE_MAP.md) 是 **能力清单与实现状态快照**，用于展示平台能力版图和当前实现成熟度；它不是本 PRD 的替代品，也不直接等同于 blackboard gate 的批准状态。
- 本文档不冻结 API、数据库 schema、模型输入输出或任务编排细节，这些内容由 `architect` 在 `architecture/api-contract/data-model-contract` 文档中进一步定义。

### 1.2 优先级定义

| 优先级 | 含义 | 产品要求 |
| :--- | :--- | :--- |
| **P0** | 当前版本必须成立的核心闭环 | 缺失会直接影响当前版本的核心价值与用户可用性 |
| **P1** | 当前版本重要增强能力 | 可提升体验和差异化，但不应阻塞核心闭环上线 |
| **P2** | 后续阶段或选配能力 | 保留产品方向，不作为当前 gate 的前置条件 |

### 1.3 状态口径定义

| 状态 | 含义 |
| :--- | :--- |
| **已上线 / 已实现** | 仓库或当前产品版本中已经存在对应能力实现 |
| **当前阶段** | 属于本阶段重点验收和持续完善范围 |
| **部分实现** | 已有部分功能或技术基础，但还未形成完整用户价值闭环 |
| **规划中** | 已纳入路线图，但不作为当前阶段验收前提 |

> 说明：**优先级** 表示业务重要性，**状态** 表示实现成熟度，二者是两个独立维度，不能互相替代。

### 1.4 当前版本验收范围说明

- 本次 PM 口径对齐后，当前版本的 **P0 核心闭环** 定义为：
  - 智能体检录入与分析
  - Dr. AI 智能顾问（RAG）
  - 多维风险评估与风险解释
- `全周期健康时光机` 属于 **P1 重点增强能力**，应持续完善，但不单独阻塞 P0 核心闭环成立。
- `精准干预`、`亲情账户`、`药物基因组学`、`IoT 实时监控` 等能力保留在产品蓝图内；若已实现，则视为现有增强能力，不因此改变其产品优先级定义。
...
- 用户可成功上传符合限制条件的 PDF/JPG/PNG 体检报告，系统明确提示不合规文件。
- 对于标准体检报告，系统能输出结构化字段结果，并允许用户在提交前人工确认或修正。
- OCR、LLM、正则兜底、手动录入之间的降级路径明确可见，失败时不会让用户卡死在无反馈状态。
- 结构化结果写入个人档案后，可触发后续风险分析流程，而不是停留在孤立上传记录。
...
- 用户可在问答场景中获得与自身健康画像相关的回答，而不是脱离上下文的泛泛建议。
- 回答必须附带可识别的指南或知识来源说明，满足“可追溯”要求。
- 当知识库命中不足、模型异常或缓存失效时，系统仍能给出可理解的失败反馈，不得静默返回不可判断结果。
- 当前版本以“有依据的健康问答”成立为准，不以复杂多轮诊疗代理或自动闭环决策作为验收前提。
...
- 用户能够按时间线查看历史健康记录，并区分不同数据来源。
- 核心指标趋势图可正常展示，且用户能够理解趋势而非只看到原始数字堆叠。
- 若未来推演能力开启，必须清楚标记其为模拟结果，并与当前真实记录区分展示。
- 本模块属于 P1 增强能力，未覆盖的预测细节不阻塞 P0 核心闭环验收。
...
- 本模块保留在产品蓝图中，属于增强型能力，不作为当前 `prd_ready` 的核心阻塞项。
- 若仓库当前已具备部分实现，应在 `FEATURE_MAP` 中作为“已实现能力快照”呈现，但仍按 P2 管理其业务优先级。
- 后续若该模块要提升为当前阶段重点验收项，需要通过 PM 明确调整优先级和版本目标。
...
> 路线图说明：本节描述的是产品阶段规划，不直接等同于 blackboard 中的 gate 状态；gate 是否打开取决于对应阶段文档是否完成并被正式批准。
