    ...
    <div class="p-6 h-full flex flex-col items-center overflow-auto">
        <div class="w-full max-w-5xl">
            ...
            <el-tabs v-model="activeTab" class="profile-tabs">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-4">
                        ...
                        <GlassCard :glowProximity="100">
                            ...
                            <div class="flex flex-col items-center py-8">
                                <el-avatar :size="80"
                                    class="bg-linear-to-br from-blue-500 to-purple-600 text-white text-3xl font-bold shadow-lg">
                                ...
                                <el-tag :type="authStore.user?.is_superuser ? 'danger' : 'info'" effect="dark" class="mt-3" round>
                                    {{ authStore.user?.is_superuser ? '🛡️ 管理员' : '👤 普通用户' }}
                                </el-tag>
                        ...
                        <GlassCard :glowProximity="100" v-if="!authStore.user?.is_superuser">
                            ...
                            <div v-else-if="!hasProfile" class="flex flex-col items-center py-12">
                                ...
                                <p class="text-slate-500 dark:text-slate-400 text-center">
                                    暂无健康档案<br><span class="text-sm">点击上方按钮开始建立您的档案</span>
                                </p>
                            </div>
                ...
                <!-- Tab 2: Medical Documents (Task 57) - Hidden for Admins (Task 82) -->
                <el-tab-pane label="电子病历夹" name="documents" v-if="!authStore.user?.is_superuser">
                                ...
                                <div class="flex items-center justify-between bg-white/50 dark:bg-black/20 p-3 rounded-lg border border-slate-100 dark:border-white/5">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="w-10 h-10 flex items-center justify-center rounded-lg" 
                                            :class="isPdf(doc.file_name) ? 'bg-red-100 dark:bg-red-900/30' : 'bg-blue-100 dark:bg-blue-900/30'">
                                        ...
                                        <div>
                                            ...
                                            <div class="text-xs text-slate-500">
                                                <el-tag v-if="doc.has_data" type="success" size="small">已提取数据</el-tag>
                                                <el-tag v-else type="info" size="small">未提取数据</el-tag>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="flex gap-2">
                                        ...
                                        <el-button v-if="doc.has_data" size="small" type="success" @click="loadToAnalysis(doc)">
                                            <el-icon class="mr-1"><DocumentCopy /></el-icon> 载入
                                        </el-button>
...
import { Loading, Upload, View, DocumentCopy, Delete } from '@element-plus/icons-vue'
...
const { userProfile: profile, geneStats, riskReport } = storeToRefs(healthStore)
...
// Task 61: Format Date to Local Time
...
const fetchDocuments = async () => {
    ...
    try {
        const res = await axios.get('http://127.0.0.1:8000/api/v1/user/documents')
...
const viewDocument = (doc) => {
    const url = `http://127.0.0.1:8000${doc.file_url}`
    window.open(url, '_blank')
}

// Load to Analysis (Task 59)
const loadToAnalysis = (doc) => {
    if (!doc.ocr_summary) {
        showToast('该文件无结构化数据', 'warning')
        return
    }
    healthStore.setImportData(doc.ocr_summary)
    ElNotification.success({ title: '数据已准备', message: '正在跳转到临床体检页面...' })
    router.push('/clinical')
}

// Task 70: Delete Document
const deleteDocument = async (doc) => {
    try {
        ...
        const res = await axios.delete(`http://127.0.0.1:8000/api/v1/user/documents/${doc.id}`)
        if (res.data.status === 'success') {
